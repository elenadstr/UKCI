...
def run_eca(seriesA, seriesB, delT=4, tau=1, blocks=None, years=None,
    ...
    a = np.asarray(seriesA, dtype=int)
    ...
    if len(blks) == 1 and len(blks[0]) == len(a):
        ...
        return eca_ts(a, np.asarray(seriesB, dtype=int), delT=delT, tau=tau,
                      sigtest=sigtest)
    return eca_blocked(a, np.asarray(seriesB, dtype=int), blks, delT=delT,
