...
def get_annual_stats(events_df, year_col='year', ensemble_col=None,
    ...
    if months_col is not None:
        from .detector import season_months
    ...
    keys = [ensemble_col, year_col] if ensemble_col else [year_col]
    grouped = events_df.groupby(keys, sort=False)

    agg = grouped.size().rename('n_events').to_frame()
    if duration_col is not None:
        d = grouped[duration_col].agg(['sum', 'mean', 'max'])
        d.columns = ['total_duration', 'mean_duration', 'max_duration']
        agg = agg.join(d)
    if length_col is not None:
        l = grouped[length_col].agg(['sum', 'mean', 'max'])
        l.columns = ['total_length', 'mean_length', 'max_length']
        agg = agg.join(l)

    if ensemble_col:
        ensembles = sorted(events_df[ensemble_col].unique())
        full_index = pd.MultiIndex.from_product(
            [ensembles, years], names=[ensemble_col, year_col]
        )
    else:
        full_index = pd.Index(years, name=year_col)

    out = agg.reindex(full_index)
    out['n_events'] = out['n_events'].fillna(0).astype(int)
    for c in ('total_duration', 'max_duration', 'total_length', 'max_length'):
        if c in out.columns:
            out[c] = out[c].fillna(0).astype(int)

    out = out.reset_index()
    if ensemble_col:
        out = out.rename(columns={ensemble_col: 'ensemble', year_col: 'year'})
    else:
...
before any cross-variable compounding or ECA linkage.
    ...
    pd.DataFrame
        ...
        :func:`get_annual_stats`.
    """
    from .detector import (make_season_blocks,
                           season_year_labels)
...
def get_hovmoller_data(events_df, ensemble_col='ensemble', year_col='year',
    ...
    keys = [ensemble_col, year_col]
    grouped = events_df.groupby(keys, sort=False)
    full_index = pd.MultiIndex.from_product(
        [ensembles, years], names=[ensemble_col, year_col]
    )

    out = grouped.size().rename('n_events').to_frame().reindex(full_index)
    out['n_events'] = out['n_events'].fillna(0).astype(int)

    if case_col is not None:
        d = grouped[case_col].agg(['sum', 'max', 'mean'])
        d.columns = ['n_extreme_cases', 'max_duration', 'mean_duration']
        out = out.join(d)
        out['n_extreme_cases'] = out['n_extreme_cases'].fillna(0).astype(int)
        out['max_duration'] = out['max_duration'].fillna(0).astype(int)

        dur = grouped[case_col].apply(lambda s: [int(x) for x in s.to_numpy()])
        out = out.join(dur.rename('durations'))
        out['durations'] = out['durations'].apply(
            lambda v: v if isinstance(v, list) else []
        )

    if length_col is not None:
        out = out.join(grouped[length_col].mean().rename('mean_length'))

    for c in extra_cols:
        out = out.join(grouped[c].sum().rename(c))
        out[c] = out[c].fillna(0).astype(int)

    out = out.reset_index().rename(columns={ensemble_col: 'ensemble',
                                            year_col: 'year'})
    return out
...
def plot_duration_and_counts(events_df, ensemble_col='ensemble', year_col='year', length_col='length', year_range=(1979, 2079), cmap='YlGnBu_r', figsize=(16, 10)):
    ...
    filtered = events_df[(events_df[year_col] >= start_yr)
                         & (events_df[year_col] <= end_yr)]
    ensembles = sorted(events_df[ensemble_col].unique())
    n_ensembles = len(ensembles)
    max_length = int(events_df[length_col].max())

    duration_matrix = (
        filtered.groupby([length_col, year_col]).size().unstack(fill_value=0)
        .reindex(index=np.arange(max_length + 1), columns=years, fill_value=0)
        .to_numpy(dtype=float)
    )

    count_matrix = (
        filtered.groupby([ensemble_col, year_col]).size().unstack(fill_value=0)
        .reindex(index=ensembles, columns=years, fill_value=0)
        .to_numpy(dtype=int)
    )
