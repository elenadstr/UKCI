...
def make_season_blocks(years=None, months=None, season_start=6,
    ...
    idx = np.flatnonzero(in_season)
    if idx.size == 0:
        return []

    # labels[idx] is chronological; split where the season-year label changes.
    labs = labels[idx]
    cut = np.flatnonzero(labs[1:] != labs[:-1]) + 1
    return [np.asarray(b, dtype=int) for b in np.split(idx, cut)]
...
def flag_extreme_events(timeseries, threshold, N=1, min_days=None,
    ...
    flag : {'last', 'first', 'all', 'combined'}, optional
        Which day(s) of each qualifying window to flag.
        ...
        - ``'combined'``: return a dictionary with the three arrays that
          would be returned by ``flag='first'``, ``flag='all'``, and
          ``flag='last'``.

        Caveats: ``'last'``/``'first'`` mark the window edge, which with
        ``min_days < N`` may itself be a non-extreme day; ``'all'`` leaves
        a spell tail shorter than a full window unflagged. The scan jumps
        past each qualifying window (``i += N``): overlapping qualifying
        windows are not re-flagged, so one long spell yields one flag per
        N days.

    Returns
    -------
    numpy.ndarray or dict
        If ``flag`` is ``'last'``, ``'first'``, or ``'all'``, returns a
        binary array with 1 = extreme event, 0 = not.

        If ``flag`` is ``'combined'``, returns a dictionary with keys:
        ``'first'``, ``'all'``, ``'last'``.
    """
    timeseries = np.asarray(timeseries, dtype=float)
    if timeseries.ndim != 1:
        raise ValueError(f"timeseries must be 1-D, got shape {timeseries.shape}")
    ...
    if flag not in ('last', 'first', 'all', 'combined'):
        raise ValueError(
            f"flag must be 'last', 'first', 'all' or 'combined', got '{flag}'"
        )

    exceeds = (timeseries > threshold) if direction == 'above' else (timeseries < threshold)
    n = len(timeseries)

    if N == 1:
        base = exceeds.astype(np.uint8)
    ...
    # Fast window-hit test using cumulative sums instead of repeated slices.
    ex_i = exceeds.astype(np.int32, copy=False)
    csum = np.empty(n + 1, dtype=np.int32)
    csum[0] = 0
    np.cumsum(ex_i, out=csum[1:])
    window_counts = csum[N:] - csum[:-N]
    qualifies = window_counts >= min_days

    if flag == 'combined':
        events_first = np.zeros(n, dtype=np.uint8)
        events_all = np.zeros(n, dtype=np.uint8)
        events_last = np.zeros(n, dtype=np.uint8)

        i = 0
        while i < valid_starts:
            if qualifies[i]:
                events_first[i] = 1
                events_last[i + N - 1] = 1

                window = exceeds[i:i + N]
                segment = events_all[i:i + N]
                segment[window] = 1
...
def detect_compound_events(binary_series, delT=4, tau=1, min_duration=2,
    ...
    return pd.DataFrame(rows,
                        columns=["start_idx", "end_idx", "length",
                                 "n_extreme_cases"])

def _cluster_within_blocks(indices, blocks, max_gap, n_steps=None):
    ...
    if n_steps is None:
        max_pos = max((int(np.max(blk)) for blk in blocks if len(blk)),
                      default=-1)
        n_steps = max_pos + 1
    block_id = np.full(int(n_steps), -1, dtype=np.int32)
    for bi, blk in enumerate(blocks):
        b = np.asarray(blk, dtype=int)
        if b.size:
            block_id[b] = bi
    groups, current = [], []
    for idx in indices:
        idx = int(idx)
        if idx < 0 or idx >= block_id.size or block_id[idx] < 0:
...
def detect_compound_events_bivariate(binary_1, binary_2, delT=4, tau=1,
    ...
    union_idx = np.flatnonzero(b1 | b2)
